...
class PHPPaging {
    ...
    function linkClase($id) {
        if (empty($id))
            return $this->error(true, "El valor indicado en el m�todo linkClase() est� vac�o");
        if (!preg_match("/(^[a-zA-Z0-9]+)$/", $id) or $id === true)
    ...
    function fetchResultado() {
        ...
        if (is_array($this->ejecutard)) {
            if (list($key, $row) = each($this->ejecutard))
                return $row;
        } elseif ($row = @mysqli_fetch_array($this->ejecutard, MYSQLI_BOTH)) {
            return $row;
        } else
            return false;
    }
    ...
    function ejecutar() {
        ...
        if (!empty($this->sql)) {
            $result = (isset($this->conn)) ? @mysqli_query($this->conn, $this->sql) : @mysqli_query($this->conn, $this->sql);
            if (!$result)
                return $this->error(false, @mysqli_error(), $this->sql); // version 2.0
        ...
        if ($this->numTotalRegistros >= 1) {
            ...
            if (!empty($this->sql)) {
                ...
                $result = (isset($this->conn)) ? @mysqli_query($this->conn, $sql) : @mysqli_query($this->conn, $sql);
                if (!$result)
                    return $this->error(false, @mysqli_error(), $sql, preg_match("limit[ ]+[0-9]+(,[ ]*[0-9]+)?", strtolower($sql))); // version 2.0
    ...
    function error($desarrollo = true, $msg, $query = false, $limit_error = null) {
        if ($this->modo != 'publicacion' and $this->mostre_error == false) {
            if ($this->modo == 'reporte') {
                ...
                if ($limit_error == true)
                    $error.= "El error probablemente se deba a que en la consulta MySQL que usted indic� aparentemente ya exist�a una cl�usula LIMIT, la cu�l es a�adida autom�ticamente por el paginador. M�s informaci�n en la p�gina web del script.<br /><br />";
